...
public class BestConfigurationDataExtraction {
    ...
    private static final OutputMode OUTPUT_MODE = OutputMode.BEST_CONFIGS;
    ...
    private void run(String corpus) throws IOException {
        ...
        for (String p : numericalParams) {
            ...
            if (OUTPUT_MODE == OutputMode.BEST_PARAM_OVERVIEW)
                System.out.println("[" + corpus + "] " + p + " - avg: " + average.getAsDouble() + ", stddev: " + Math.sqrt(variance));
            if (OUTPUT_MODE == OutputMode.BEST_CONFIGS)
        ...
        for (String p : booleanParams) {
            ...
            if (OUTPUT_MODE == OutputMode.BEST_CONFIGS && count >= 5)
                System.out.println("[" + corpus + "] " + p + " = true");

        }
        Set<String> matchTypeParams = bestEntries.stream().map(e -> e.getRunInfo().getConfiguration().getSettings())
                .flatMap(m -> m.keySet().stream().filter(k -> {
                    try {
        ...
        for (String p : matchTypeParams) {
            ...
            if (OUTPUT_MODE == OutputMode.BEST_CONFIGS && phrasecount >= 5)
                System.out.println("[" + corpus + "] " + p + " = phrase");
            ...
            if (OUTPUT_MODE == OutputMode.BEST_CONFIGS && phrasecount >= 5)
                System.out.println("[" + corpus + "] " + p + " = best_fields");
        }
        ...
        for (String p : templateParamters) {
            ...
            if (OUTPUT_MODE == OutputMode.BEST_CONFIGS) {
                if (melanoma >= 5)
                    System.out.println("[" + corpus + "] " + p + " = " + bestEntries.stream().map(e -> e.getRunInfo().getConfiguration().getSettings().get(p)).filter(t -> t.contains("melanoma")).findAny().get());
                if (dismax >= 5)
                    System.out.println("[" + corpus + "] " + p + " = " + bestEntries.stream().map(e -> e.getRunInfo().getConfiguration().getSettings().get(p)).filter(t -> t.contains("dismax")).findAny().get());
                if (defaulttemplate >= 5)
                    System.out.println("[" + corpus + "] " + p + " = " + bestEntries.stream().map(e -> e.getRunInfo().getConfiguration().getSettings().get(p)).filter(t -> t.contains("generic.json")).findAny().get());
            }
        }
    }

    private enum OutputMode {BEST_CONFIGS, BEST_PARAM_OVERVIEW}
}
